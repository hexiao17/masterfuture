 ...
 *
 */
class ZbnewsController extends BaseAdminController
{
    ...
    public function actionInfo(){
        ...
        //收藏历史       
        $fav_log_list = NewsFav::find()->where(['newlist_id'=>$id,'type'=>"zb"])->orderBy([ 'id' => SORT_DESC ])->asArray()->all();
        $fav_newlist_data =[];
       // var_dump($fav_log_list);exit();
        ...
        $attachs = null;
        if($info->file_key){            
            $attachs = ZbnewsAttach::find()->where(['oldnews_id'=>$info->file_key])->all();
        }        
    ...
    public function actionSet(){
        if( \Yii::$app->request->isGet ) {
            ...
            $tmpnews = null;
            $info = [];
            if($tmpnews_id){//就是临时新闻添加
                $tmpnews = TmpZbnews::find()->where(['id'=>$tmpnews_id])->one();
                if(!$tmpnews){
                    return $this->renderJson([],"old参数错误",-1);
                }
                
                //如果有附件就查询附件的oldnews_id
                 
                $info = [
                    'title'=>$tmpnews->title,
                    'pub_company'=>$tmpnews->flowname,
                    'pub_time'=>$tmpnews->updatetime,
                    'tags'=>'招标',
                    'expired_time'=>date("Y-m-d H:i:s"),                   
                    'content'=>$tmpnews->content,
                    'file_key'=>$tmpnews->attach_has?$tmpnews->title_md5:''
                ];
        ...
        $info = [];
        if( $id ){
            $info = ZBNews::findOne(['id' => $id]);
        }
        if( $info ){
            $model_book = $info;
        }else{
            $model_book = new ZBNews();
            $model_book->status = 1;
        ...
        $model_book->file_key = $file_key;
        //作者
        $model_book->author_id = $this->current_user['uid'];
        $succ = $model_book->save( 0 );
       
         
        if($tmpnews_id>1 && $succ){//更新中间库的状态
             
            $tmp = TmpZbnews::find()->where(['id'=>$tmpnews_id])->one();
            $tmp->statu =2;
            $tmp->save(0);
        }
