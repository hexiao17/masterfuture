...
class BookController extends BaseAdminController{
    ...
    public function actionIndex(){
        ...
        $role_mapping= Role::find()->orderBy(['pos'=>SORT_ASC])->indexBy('id')->all();
		$data = [];

		if( $list ){
			foreach( $list as $_item ){
				$tmp_cat_info = isset( $cat_mapping[ $_item['cat_id'] ] )?$cat_mapping[ $_item['cat_id'] ]:[];
				$tmp_role_info = isset($role_mapping[$_item['related_role_id']])?$role_mapping[$_item['related_role_id']]:[];
				$data[] = [
					'id' => $_item['id'],
					'name' => UtilService::encode( $_item['name'] ),
				    'origin_price' => UtilService::encode( $_item['origin_price'] ),
				    ...
				    'role_name'=>$tmp_role_info?UtilService::encode($tmp_role_info['name']):''
				];
			}
		}
    ...
    public function actionInfo(){
		...
		$reback_url = UrlService::buildAdminUrl("/book/index");
		if( !$id ){
	...
	public function actionSet(){
		if( \Yii::$app->request->isGet ) {
			...
			$role_list = Role::find()->orderBy(['pos'=>SORT_ASC])->all();
		...
		$related_role_id = intval($this->post("related_role",0));
		$name = trim( $this->post("name","") );
		
		$price = floatval( $this->post("price",0) );
		$origin_price = floatval($this->post("origin_price",0));
		...
		if( !$related_role_id ){
		    return $this->renderJSON([],"请选择角色~~",-1);
		}
		...
		if( $origin_price <= 0  ){
		    return $this->renderJSON([],"请输入符合规范的商品原价~~",-1);
		}
		...
		if( mb_strlen( $tags,"utf-8" ) < 1 ){
			return $this->renderJSON([],"请输入商品标签，便于搜索~~",-1);
		}
		...
		$model_book->related_role_id = $related_role_id;
		$model_book->name = $name;
		$model_book->price = $price;
		$model_book->origin_price = $origin_price;
