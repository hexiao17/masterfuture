...
class PayOrderService extends  BaseService {
	...
	public static function createPayOrder( $member_id,$items = [],$params = []){
		...
		try{
			//为了防止并发 库存出问题了，select for update（不是高并发）
			//第二种高并发的解决方法
			//stock = 5
			//update book set  stock = 2 where id=1 and stock=5
			//update book set  stock =3  where id=1 and stock 5
			$tmp_book_table_name = Book::tableName();
			$tmp_book_ids = array_column( $items,'target_id' ); //php 5.5+
			//通过加 for update 锁数据库来控制并发操作
