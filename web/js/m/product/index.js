...
var product_index_ops = {
    ...
    eventBind:function(){
        ...
        $( window ).scroll( function() {
            if( ( ( $(window).height() + $(window).scrollTop() ) > $(document).height() - 20 ) && process ){
                ...
                $.ajax({
                    url:common_ops.buildWebUrl( "/product/search" ),
                    type:'GET',
                    dataType:'json',
                    data:data,
                    success:function( res ){
                        ...
                        for( idx in res.data.data ){
                            ...
                            html += '<li> <a href="' + common_ops.buildWebUrl( "/product/info",{ id:info['id'] } ) + '"> <i><img src="'+ info['main_image_url'] +'"  style="width: 100%;height: 200px;"/></i> <span>'+ info['name'] +'</span> <b><label>月销量' + info['month_count'] +'</label>¥' + info['price'] +'</b> </a> </li>';
                        }
    ...
    search:function(){
        ...
        window.location.href = common_ops.buildWebUrl("/product/index",params);
    }
};
