...
var book_set_ops = {
    ...
    eventBind:function(){
        ...
        $(".wrap_book_set .save").click( function(){
            ...
            var origin_price_target = $(".wrap_book_set input[name=origin_price]");
            var origin_price = origin_price_target.val();

            var price_target = $(".wrap_book_set input[name=price]");
            var price = price_target.val();

            var summary  = $.trim( that.ue.getContent() );
            var summary_target = $(".wrap_book_set textarea[name=summary]");
            ...
            var related_role_target  = $(".wrap_book_set select[name=related_role_id]");
            var related_role = related_role_target.val();
            
            if( parseInt( cat_id ) < 1 ){
                common_ops.tip( "请选择产品分类~~" ,cat_id_target );
                return;
            }         
            if( parseInt( related_role ) < 1 ){
                common_ops.tip( "请选择角色~~" ,related_role_target );
                return;
            }
            if( name.length < 1 ){
                common_ops.alert( "请输入符合规范的产品名称~~" );
                return;
            }
            if( parseFloat( origin_price ) <=0 ){
                common_ops.tip( "请输入符合规范的产品售卖价格~~" ,origin_price_target );
                return;
            } 
            if( parseFloat( price ) <= 0 ){
                common_ops.tip( "请输入符合规范的产品售卖价格~~" ,price_target );
                return;
            }
            ...
            if( summary.length < 10 ){
                common_ops.alert( "请输入产品描述，并不能少于10个字符~~"  );
                return;
            }
            ...
            $.ajax({
                url:common_ops.buildAdminUrl("/book/set") ,
                type:'POST',
                data:data,
                dataType:'json',
                success:function(res){
                    ...
                    if( res.code == 200 ){
                        callback = function(){
                            window.location.href = common_ops.buildAdminUrl("/book/index");
                        }
                    }
